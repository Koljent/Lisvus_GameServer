...
public class EventHandleManager extends Manager
{
	...
	private static EventHandleManager _instance;

	public static EventHandleManager getInstance()
	{
		if (_instance == null)
		{
			_instance = new EventHandleManager();
		}
		return _instance;
	}
	...
	public <T extends EventObject> void dispatchEvent(EventType type, T event)
	{
		if (!_handlerTypes.containsKey(type))
		{
			_log.warning(getClass().getSimpleName() + ": Attempt to dispatch event for type " + type.name() + " has failed!");
			return;
		}
