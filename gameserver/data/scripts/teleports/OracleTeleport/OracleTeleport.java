...
public class OracleTeleport extends Quest
{
	...
	public String onTalk(L2NpcInstance npc, QuestState st)
	{
		...
		if (Util.contains(TOWN_DAWN, npcId))
		{
			...
			// Set teleport id
			for (int i = 0; i < TOWN_DAWN.length; i++)
			{
				if (TOWN_DAWN[i] == npcId)
				{
					st.set("id", String.valueOf(i));
					break;
				}
			}
			player.teleToLocation(-80157, 111344, -4901);
		}
		else if (Util.contains(TOWN_DUSK, npcId))
		{
			st.setState(State.STARTED);
			// Set teleport id
			for (int i = 0; i < TOWN_DUSK.length; i++)
			{
				if (TOWN_DUSK[i] == npcId)
				{
					st.set("id", String.valueOf(TOWN_DAWN.length + i));
					break;
				}
			}
			player.teleToLocation(-81261, 86531, -5157);
		}
		else if (Util.contains(TEMPLE_PRIEST, npcId) && st.getState() == State.STARTED)
		{
